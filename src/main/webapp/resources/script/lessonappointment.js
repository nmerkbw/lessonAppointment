var lessonappointment={
    //封装相关ajax的url
    URL:{
        appoint:function(lessonId,studentId){
            return '/uestc/lesson/'+lessonId+'/appoint/'+studentId;//?studentId=
        },
        verify:function(){
            return '/uestc/lesson'+'/verify';
        }
    },
    ...
    validateStudent:function(studentId,password){
        ...
        if(!studentId||!password){
            return "nothing";
        }else if(studentId.length!=12 ||isNaN(studentId)||password.length!=6 ||isNaN(password)){
            return "typerror";
        }else {
            if(lessonappointment.verifyWithDatabase(studentId, password)){
                console.log("验证成功！");
                return "success";
            }else{
                console.log("验证失败！");
    ...
    verifyWithDatabase:function(studentId,password){
        ...
        var verifyUrl=lessonappointment.URL.verify();
        $.ajax({
            type:'post',
            url:verifyUrl,
            data:params,
        ...
        init:function(params){
            ...
            if(studentId==null||password==null){
                ...
                $('#studentBtn').click(function (){
                    ...
                    var temp=lessonappointment.validateStudent(studentId,password);
                    console.log(temp);
                    if(temp=="nothing"){
                        ...
                        $.cookie('studentId', studentId, {  path: '/uestc/lesson'});
                        $.cookie('password', password, {  path: '/uestc/lesson'});
                        // 跳转到预约逻辑
                        var appointbox=$('#appoint-box');
                        lessonappointment.appointment(lessonId,studentId,appointbox);
                    }
                });
            }else{
                var appointbox=$('#appoint-box');
    ...
    appointment:function(lessonId,studentId, node){
        console.log("我执行预约的方法!" );
        node.html('<button class="btn btn-primary btn-lg" id="appointmentBtn">预约</button>');

        var appointmentUrl = lessonappointment.URL.appoint(lessonId,studentId);
